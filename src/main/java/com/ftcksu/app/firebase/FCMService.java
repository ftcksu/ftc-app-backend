...
public class FCMService {
    ...
    public void sendMessageToMultipleTokens(PushNotificationRequest request)
            throws InterruptedException, ExecutionException {
        MulticastMessage message = getPreconfiguredMulticastMessageToMultipleTokens(request);
        BatchResponse response = FirebaseMessaging.getInstance().sendMulticastAsync(message).get();
        logger.info("Sent message to multiple tokens. Success count: " + response.getSuccessCount() +
                ", Failure count: " + response.getFailureCount());
    }
    ...
    private MulticastMessage getPreconfiguredMulticastMessageToMultipleTokens(PushNotificationRequest request) {
        return getPreconfiguredMulticastMessageBuilder(request).addAllTokens(request.getTokens())
                .build();
    }
